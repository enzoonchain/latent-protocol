...
```bash
curl -X POST https://api.latentprotocol.xyz/campaign/create \
...
```bash
curl -X POST https://api.latentprotocol.xyz/campaign/{campaign_id}/ad \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Earn 12% APY",
...
```bash
curl -X POST https://api.latentprotocol.xyz/campaign/{campaign_id}/buy \
  -H "Content-Type: application/json" \
  -d '{ "blocks": 10 }'
```
...
```bash
curl https://api.latentprotocol.xyz/campaign/{campaign_id}
```
...
```bash
curl "https://api.latentprotocol.xyz/campaign/?wallet=0xYOUR_WALLET"
```
