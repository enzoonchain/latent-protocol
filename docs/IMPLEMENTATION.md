...
- **2026-06-13** — **Campaign/ad CRUD + block bidding backend** (#12, P2). `server/routes/campaigns.py` now writes to Postgres: create campaign (min $10 budget, registers advertiser), add ad creative, fund/top-up (block purchase, reactivates exhausted), get campaign + serving stats (impr/clicks/CTR/blocks remaining), list by advertiser. `server/auction.py` pure helpers (impressions/blocks affordable, block cost) + 4 tests. Serve-side bid ranking already in `matcher` (bid×relevance). x402-gated funding still pending (3.2.5). This unblocks the serve loop — ads can now exist in the DB.
- **2026-06-13** — **Integrity & anti-fraud landed** (#17, P1). Signed impression tokens (`server/security.py`, HMAC, env `IMPRESSION_SIGNING_SECRET`): `/ad/request` issues a token bound to (ad_id, wallet); `/ad/impression` rejects forged/expired tokens (403). Anti-replay dedup guard in `log_impression` (no double-log within `IMPRESSION_REPLAY_WINDOW_SECONDS`). Click anti-misclick gate (`CLICK_MIN_VIEW_SECONDS`) + existing first-click idempotency. Clients (MCP server, Hermes adapter) echo the token. 5 new unit tests (21 total).
